/*
** Module: openiod-fiware-connect-ttn.js
...
var data = require("ttn").data;
var application = require("ttn").application;
...
//	for (var i=0;i<_result.length;i++){
		var sourceData 				= _result.payload_fields;
		sourceData.dev_id			= _result.dev_id;
		sourceData.time 			= _result.metadata.time;
...
module.exports = {
    ...
    _sourceAppId        = _source.appId;
    _sourceAccessKey    = _source.accessKey;
		_sourceDevIds				= _source.devIds;
    ...
    //const appID = "2019arbaairaqsensor1"
    //const accessKey = "ttn-account-v2.-i9mJqb9YC7XP522sPYrkRwJPU96xOtnKuMNKZiaBdE"

    // discover handler and open mqtt connection
    data(_sourceAppId, _sourceAccessKey)
      .then(function (client) {
        client.on("uplink", function (devID, payload) {
					if (_sourceDevIds[devID] == undefined) {
						console.log('Device ' + devID + ' not defined, skipped.' );
						return;
					}
					if (_sourceDevIds[devID] == false) {
						console.log('Device ' + devID + ' defined but not active, skipped.' );
						return;
					}
          console.log("Received uplink from ", devID)
					if (payload.payload_fields == undefined) {
						console.dir(payload);
						console.log('ERROR: payload_fields missing');
						return;
					} else {
